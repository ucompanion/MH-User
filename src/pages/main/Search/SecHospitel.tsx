...
const SecHospitel = () => {
    const [selectedKeywordId, setSelectedKeywordId] = useState('item1')
    const handleKeywordClick = (id:string) => {
        setSelectedKeywordId(id); // 클릭된 버튼의 ID로 상태 업데이트
    };
	return (
                ...
                <ScrollContainer className="mediCate-scroll">
                    <ul className='mediCate-list'>
                        {hospitelKeywordData.map((item) => (
                            <li key={item.id}>
                                <button type='button'
                                    className={`btn mediCate-tab ${item.id === selectedKeywordId ? 'is-selected' : ''}`}
